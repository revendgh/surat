...
@foreach($data as $item)
        ...
        <div class="row">
            <div class="col text-justify">
                ...
                kuesioner di {{$item->instansi}} pada {{$item->tgl_awal}} s.d {{$item->tgl_akhir}} untuk keperluan data
                tugas mata kuliah " 
                @foreach($matkul as $data => $matakuliah)
                    @if ($data == 0)
                        {{$matakuliah->mata_kuliah}}
                    @elseif ($data == $count_matkul)
                        dan {{$matakuliah->mata_kuliah}}
                    @else
                        , {{$matakuliah->mata_kuliah}}
                    @endif
                @endforeach 
                " atas nama yang bersangkutan. Selanjutnya mohon diberikan bantuan 
                kepada mahasiswa dimaksud.
